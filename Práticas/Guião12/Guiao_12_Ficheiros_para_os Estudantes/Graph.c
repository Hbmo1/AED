...
int GraphRemoveEdge(Graph* g, unsigned int v, unsigned int w) {
  ...
  ListMove(g->verticesList, v);
  ListMove(g->verticesList, w);
  // Verificar a presença dos 2 vertices no grafo e verificar se existe uma aresta entre os dois
  // Se sim, remover a aresta e decrementar o numero de arestas.
  // Se for um grafo não direcionado, remover a aresta no sentido contrário também.
