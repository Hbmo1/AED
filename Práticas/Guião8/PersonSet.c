...
PersonSet *PersonSetCreate() {
  ...
  // COMPLETE ...
  struct _PersonSet_* ps = (struct _PersonSet_*) malloc(sizeof(*ps));
...
void PersonSetDestroy(PersonSet **pps) {
  ...
  free((*pps)->array);  // free the array of Persons
  free(*pps); //Free the PersonSet
  *pps = NULL; //Set the pointer to NULL to avoid dangling pointers
}
...
Person *PersonSetPop(PersonSet *ps) {
  ...
  Person* personToPop = (ps->array[--(ps->size)]);
...
Person *PersonSetRemove(PersonSet *ps, int id) {
  ...
  int Pfound = search(ps,id);
  if (Pfound == -1) { return NULL; }
  ps->array[ps->size] = NULL;
  // Pegar na ultima person do array para tapar o buraco
